...
export const Pagination = ({ totalCount, pageSize, currentPage, pagesCount, onChangePage, categoryGenitiveName, }) => {
    ...
    const prevPage = (_jsx(Button, { onClick: () => onChangePage(currentPage - 1), disabled: currentPage <= 1, icon: _jsx(ChevronLeft, { size: 24 }) }));
    const firstPage = (_jsx(Button, { onClick: () => onChangePage(1), variant: 'outline', toggled: currentPage == 1, children: "1" }));
    ...
    const lastPage = pagesCount > 1 ? (_jsx(Button, { onClick: () => onChangePage(pagesCount), variant: 'outline', toggled: currentPage == pagesCount, children: pagesCount })) : undefined;
    const nextPage = (_jsx(Button, { onClick: () => onChangePage(currentPage + 1), disabled: currentPage >= pagesCount, icon: _jsx(ChevronRight, { size: 24 }) }));
    ...
    return (_jsxs("ul", { className: 'pagination', children: [_jsxs("div", { className: 'pagination__numbers', children: [prevPage, firstPage, startDots, pagesButtons.map((page) => {
                        return (_jsx(Button, { onClick: () => onChangePage(page), variant: 'outline', toggled: currentPage === page, children: page }, page));
