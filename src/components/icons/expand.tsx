...
interface IExpandProps extends ISquareIconProps {
  rotated?: boolean;
}

export function Expand({ className = null, color = DEFAULT_ICON_COLOR, size = 18, rotated = false }: IExpandProps) {
  const classNames = ['expand-icon'];
  if (rotated) classNames.push('expand-icon_rotated');
